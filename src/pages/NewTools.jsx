...
export default function NewTools() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-10 lg:py-16">
      ...
      {tools.length === 0 ? (
        <p className="mt-10 text-sm text-zinc-400">No new tools in the last 30 days — check back soon.</p>
      ) : (
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {tools.map((tool) => (
            <div key={tool.slug} className="glass rounded-2xl p-5">
              <span className="flex items-center justify-between gap-1.5 text-[10px] font-bold text-zinc-400">
