...
export default function StackCost({ tools }) {
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-1.5" role="note" aria-label={`Stack cost: ${summary}`} title={summary}>
      ...
      {b.unknown > 0 && (
        <span className="rounded-full border-2 border-slate-600 px-2 py-0.5 font-display text-[10px] font-black uppercase tracking-wider text-slate-400" aria-hidden="true">
          {b.unknown} unpriced
        </span>
      )}
      {!b.paid && !b.unknown && (
