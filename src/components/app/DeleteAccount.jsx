          ...
          >
            {step === 'warn' && (
              <>
                ...
                <div className="mt-4 grid gap-3 text-xs leading-relaxed">
                  <div>
                    <p className="font-bold text-rose-300">Erased</p>
                    <p className="mt-1 text-zinc-400">Your profile and quiz answers, stack and saved tools, roadmap progress, alert emails, and your sign-in.</p>
                  </div>
                  <div>
                    <p className="font-bold text-zinc-300">Kept, without your name</p>
                    <p className="mt-1 text-zinc-400">Records of past payments — amount, plan and date only — because tax law requires them. Your email and phone are removed from them.</p>
                  </div>
                  {plan?.active && (
                ...
                <p className="mt-4 text-xs text-zinc-400">We will email a 6-digit code to confirm it is you.</p>
                ...
                <div className="mt-5 flex flex-wrap justify-end gap-3">
                  ...
                  <button
                    ...
                    className="min-h-11 rounded-md border border-white/10 bg-rose-500 px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-50"
                  >
            ...
            {step === 'code' && (
              <form onSubmit={confirm}>
                ...
                <label htmlFor="delete-code" className="mt-4 block text-xs font-bold uppercase tracking-wide text-zinc-400">Code</label>
                <input
                  id="delete-code"
                  ref={firstRef}
                  value={code}
                  ...
                  maxLength={7}
                  disabled={busy}
                  className="mt-1.5 w-full rounded-md border border-white/10 bg-zinc-900 px-3 py-3 text-center font-mono text-2xl tracking-[0.4em] text-white tabular-nums focus-visible:outline focus-visible:outline-2 focus-visible:outline-rose-300"
                  placeholder="••••••"
                />
                {error && <p role="alert" className="mt-3 text-xs font-semibold text-rose-400">{error}</p>}
