...
GREEN means all three pass:
  npm test        (node --test in radar/)
  npm run build   (vite build + service-worker stamping)
  npm run smoke   (renders all 11 routes headless, asserts 0 console errors)

GitHub access is through the github MCP tools (mcp__github__*), which are
authenticated. Load them with ToolSearch. Do NOT use the `gh` CLI — it is not
set up in this sandbox.

The smoke script now resolves whatever chromium build the sandbox has and waits
for vite preview to answer before testing. If it still fails on a browser path
or a connection refusal, that is a bug in scripts/smoke.mjs worth fixing — do
not hand-copy browser binaries around as a workaround.
...
You may ALSO make one small real improvement if you spot something clearly
worth fixing (a demonstrable bug, an a11y or perf problem). Do not go hunting
for test coverage — the radar module has 102 tests already and more are no
longer the best use of a run.
...
SECOND, write the day up:
  - Prepend a dated section to DEVLOG.md above the previous day, covering:
    ...
    seconds. Be honest when a day produced little.
  - Post the same summary as a GitHub issue titled `Dev digest YYYY-MM-DD`
    using the github MCP issue-creation tool, and close the previous day's
    digest issue with the same toolset. If issue creation fails, keep
    DEVLOG.md and say plainly in your final message that the issue failed and
    why.
  - Commit DEVLOG.md and the backlog together and push.
...
=== SHIPPING (the only path to production) ===
  ...
  - If ANY fail: git checkout . and abandon the work. NEVER push red code.
    Do not weaken, skip, or delete a test to make it pass.
  - The repo often advances mid-run. If push is rejected, fetch and rebase onto
    origin/master, re-run the three checks, then push again.
  - Verify with git log that your push landed.
